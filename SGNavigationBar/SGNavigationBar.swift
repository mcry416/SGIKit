 ...
 */
open class SGNavigationBar: UIView {
    
    public typealias ClickAction = () -> Void
    ...
    private lazy var bottomEffect: UIImageView = self.createBottomView()
    ...
    public override init(frame: CGRect) {
        ...
        self.addSubview(bottomEffect)
        
//        self.createBottomGradient()
...
extension SGNavigationBar {
    ...
    private func createBottomView() -> UIImageView {
        let imageView = UIImageView()
        imageView.image = UIImage(named: "sg_navigation_black_effect")
        imageView.frame = CGRect(x: 0, y: kNavigationBarHight(), width: kScreenWidth(), height: 12)
        return imageView
    }
    
    private func createBottomGradient() {
        let gradientLayer = CAGradientLayer()
        gradientLayer.colors = [UIColor.gray.withAlphaComponent(0.618).cgColor,
                                UIColor.white.cgColor]
        gradientLayer.frame = CGRect(x: 0, y: kNavigationBarHight(), width: kScreenWidth(), height: 12)
        
        let shadowLayer = CALayer()
        shadowLayer.frame = gradientLayer.bounds
        shadowLayer.shadowColor = UIColor.gray.withAlphaComponent(0.618).cgColor
...
extension SGNavigationBar {
    ...
    private func createCenterLabel(text: String) -> UILabel {
        ...
        label.textColor = KitColor.black()
        label.text = text
...
extension SGNavigationBar{
    ...
    private func createRightButton(text: String) -> UIButton{
        ...
        button.frame.size = CGSize(width: CONTENT_WIDTH, height: CONTENT_WIDTH)
        if text != "" {
            button.setTitle(text, for: .normal)
        } else {
            button.setImage(UIImage(named: RIGHT_IMAGE_NAME), for: .normal)
        }
        button.setTitleColor(KitColor.black(), for: .normal)
