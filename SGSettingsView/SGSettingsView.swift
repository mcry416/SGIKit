 ...
 */
open class SGSettingsView: UIView {
...
extension SGSettingsView {
     ...
     */
    @objc
    open func getFinalHeight() -> CGFloat{
        return CGFloat(datas.count * 44)
    }
    
    @objc
    open func setCellWithSwitchStatus(at index: Int, status: Bool){
    ...
    @objc
    open func setOnSettingsUpdateEvent(_ index: Int){
        updateView(index)
    }
...
extension UITableViewCell {
    ...
    public func setSGSettingsCell(model: SettingsModel, switchBlock: ((UISwitch) -> Void)?){
        switch model.type {
        case .done:
